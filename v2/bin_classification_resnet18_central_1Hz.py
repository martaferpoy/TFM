...
experiment.set_name("ResNet18 bin classification Central 1Hz")
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        thorres_small = x_torres_ind[-100:, :100]
        abdores_small = x_abdo_ind[-100:, :100]
        
        thorres = torch.from_numpy(np.moveaxis(thorres_small / 255, -1, 0))
        abdores = torch.from_numpy(np.moveaxis(abdores_small / 255, -1, 0))
        
        x = torch.cat([thorres, abdores], axis = 0)
        x_resized = F.interpolate(x.unsqueeze(0), size=(155, 155), mode='bilinear', align_corners=False).squeeze(0)

        y = torch.tensor(0) if y_central_ind < 5 else torch.tensor(1)

        return x_resized, y, name_ind
...
model = timm.create_model('resnet18', pretrained=True, num_classes = 2, in_chans = 6).to(device)
...
torch.save(best_model.state_dict(), '/home/marta/MsC/trained_models/bin_classification_resnet18_central_1Hz.pth')


# In[97]:


experiment.log_notebook('bin_classification_resnet18_central_1Hz.ipynb')
experiment.end()
