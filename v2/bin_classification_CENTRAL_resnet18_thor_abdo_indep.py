...
experiment.set_name("ResNet18 bin classification CENTRAL independent thorres abdores")
...
def get_indep_data(data):
    data_indep = []
    for person_data in data:
        thor_dict = {
            'x': person_data['image thorres'],
            'y': person_data['central events'],
            'name': person_data['name']
        }
        abdo_dict = {
            'x': person_data['image abdo'],
            'y': person_data['central events'],
            'name': person_data['name']
        }
        data_indep.append(thor_dict)
        data_indep.append(abdo_dict)
    return data_indep
...
data_train_indep = get_indep_data(data_train)
data_validation_indep = get_indep_data(data_validation)
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        x = torch.from_numpy(np.moveaxis(x_ind / 255, -1, 0))
        y = torch.tensor(0) if y_ind < 5 else torch.tensor(1)
        return x, y, name_ind
...
dataset_train = MyDataset(data_train_indep)
dataset_validation = MyDataset(data_validation_indep)
...
torch.save(best_model.state_dict(), '/home/marta/MsC/trained_models/bin_classification_CENTRAL_resnet18_thor_abdo_indep.pth')


# In[97]:


experiment.log_notebook('bin_classification_CENTRAL_resnet18_thor_abdo_indep.ipynb')
experiment.end()
