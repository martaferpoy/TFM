...
experiment.set_name("ResNet18 bin classification CENTRAL propio FitOneCycle Ponderado Threshold 0")
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        y = torch.tensor(0) if y_central_ind == 0 else torch.tensor(1)
...
model = ResNet18(6, ResBlock, outputs=2).to(device)
...
count0 = counts[0]
count1 = counts[1]
counts_all = counts[0] + counts[1]

prop0 = round(count0/counts_all, 4)
prop1 = round(count1/counts_all, 4)

print(count0, count1, counts_all)
print(prop0, prop1)
...
class_weights = torch.FloatTensor([prop1, prop0]).cuda()
experiment.log_metric('Prop class 0', prop0)
experiment.log_metric('Prop class 1', prop1)
...
torch.save(best_model.state_dict(), '/home/marta/MsC/trained_models/bin_classification_resnet18_propio-FitOneCycle-Ponderado-Thres0-CENTRAL.pth')


# In[97]:


experiment.log_notebook('bin_classification_resnet18_propio-FitOneCycle-Ponderado-Thres0-CENTRAL.ipynb')
experiment.end()
