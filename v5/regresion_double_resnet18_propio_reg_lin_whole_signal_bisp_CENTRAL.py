...
experiment.set_name("REGRESION CENTRAL Double ResNet18 propio reg_lin BISP WHOLE SIGNAL")
...
def validate_loss_kappa_acc(val_loss, val_kappa, val_accuracy, coefs, intercept):
    ...
    with torch.no_grad():
        for data in dataloader_validation:
            inputs_thor, inputs_abdo, labels, names = data[0].to(device, dtype=torch.float32), data[1].to(device, dtype=torch.float32), data[2].to(device), data[3]
            outputs = model(inputs_thor, inputs_abdo)
...
torch.save(best_model.state_dict(), '/home/marta/MsC/trained_models/regresion_double_resnet18_propio_reg_lin_whole_signal_bisp_CENTRAL.pth')
...
with open('/home/marta/MsC/coefs/coefs_regresion_double_resnet18_propio_reg_lin_whole_signal_bisp_CENTRAL.txt', 'w') as convert_file:
...
experiment.log_notebook('regresion_double_resnet18_propio_reg_lin_whole_signal_bisp_CENTRAL.ipynb')
experiment.end()
